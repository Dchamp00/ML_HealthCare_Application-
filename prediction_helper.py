...
scalar_rest = load("artifacts/scalar_rest.joblib")
scalar_young = load("artifacts/scalar_young.joblib")
...
def preprocess_input(input_dict):
    ...
    expected_columns = [
        'age', 'number_of_dependants', 'income_lakhs', 'insurance_plan', 'genetical_risk', 'normalised_score',
...
# Assuming the 'normalized_risk_score' needs to be calculated based on the 'age'
    df['normalised_score'] = calculate_normalised_risk(input_dict['Medical History'])
...
def predict(input_dict):
    ...
    if input_dict['Age'] <= 25:
        cols = model_young.feature_names_in_
        prediction = model_young.predict(input_df[cols])
    else:
        cols = model_rest.feature_names_in_
        prediction = model_rest.predict(input_df[cols])
